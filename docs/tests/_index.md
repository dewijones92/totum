...
| "Play now, then back to what's playing" puts the chosen row in the playing slot and the playing one next, by identity, including straight after a restart | JVM unit | `:app` `PlayInsteadOfCurrentTest` (9), incl. a refused play leaving the queue as it was |
...
| Smart skip-silence: the speech detector (Silero VAD in Kotlin) matches ONNX Runtime; Smart cuts noise Standard keeps, never loud non-speech, and works when switched on mid-stream | JVM unit | `:core:playback` `SpeechModelTest` (6), `SmartCutTest` (15, incl. dense and occasional dropped-chunk gaps, a detector started mid-speech, the withheld count, the detector's own thread, late chunks, switching on mid-stream at a non-zero offset, and a replaced track going quiet); on real speech in the opt-in suite, `RealSpeechSmartTest` (6) |
| **Audio quality on real speech** (opt-in, runs in CI only when this area changes): the silence cutter and the volume boost on a public-domain LibriVox reading, as mastered and 6-30 dB down with hiss; pauses removed vs speech energy lost, a quiet guest after a loud host, words after a burst, equal loudness at every level, never a clipped sample, how cleanly the gain changes (first second, settled, whole minute), claps over quiet speech, cut then boost, chunking, stereo = mono, no clicks | `:core:playback` `…playback.audioquality.*` (19): `RealSpeechCutTest`, `RealSpeechBoostTest`, `RealSpeechChainTest`. Run with `./gradlew :core:playback:testDebugUnitTest -Ptotum.audioQualityTests` (needs `ffmpeg`). Mutation-checked: no look-ahead, over-cutting, under-cutting, no boost warm-up, an unsmoothed limiter and a doubled target each fail 2-5 tests | `.github/workflows/audio-quality.yml`, on a push touching `Silence*`, `*Boost*`, `Lookahead*`, `Speech*`, `Vad*`, the fixture, the suite or `build.gradle.kts`; preflight fails if a source the suite imports would not trigger it; a run that executes zero tests fails |
| Skip-silence really cuts: WAV/MP3/AAC podcasts, after a seek, at 2x, video, the drawn frames follow the sound, a boosted quiet podcast is still cut, and the sound breaks up at no more than two separate points | `SilenceIsReallyCutTest` (device), `SilenceCutterTest` (JVM: PipePipe's 150ms/40ms rule, bit-exact sound, faded join, exact skip count, chunk-size independence, heard-time accounting) | every commit |
...
| A grip on the REAL queue screen drags its row, about one place per row of travel | `QueueRowDragTest` (instrumented) | every commit — `ReorderAutoScrollTest` drives a synthetic list, so nothing watched where `Modifier.reorderable` is attached in the screen itself, which is exactly what broke on cbf9916 |
| A stale Undo does not put an entry back that has since been queued again | `PlaybackQueueTest` | every commit — two rows under one LazyColumn key is a crash, not a cosmetic fault; found by an Astra review of the removal change |
...
| A rewind sticks — an account figure that cannot move no longer out-ranks it for ever | `ResumeChoiceTest`, `AccountResumePositionsTest`, `RoomReconciledAccountProgressTest` | every commit — report 0.1.496 "rewind … is not working"; proven to fail with the rule removed, `expected:<0> but was:<77700>` |
| A rewind reaches the store at once, and below the five-second floor | `RoomPlaybackProgressStoreTest`, `RewindIsRememberedTest` (real player, real Room) | every commit — proven red: `expected:<1144> but was:<11273>`, and `stored position never became at least halfway` with the seek save removed |
| A chosen position says WHERE, never whether finished — so a seek or an adopted account figure neither creates nor clears Played | `RoomPlaybackProgressStoreTest` | every commit |
| Opening an item at its resume point is not recorded as a choice (`setMediaItem` does not mask as a seek) | `RewindIsRememberedTest` | every commit — verified on the emulator rather than read off the ExoPlayer source |
| One video YouTube will not track does not block everything behind it in the outbox, and is given up on | `ProgressOutboxDrainTest`, `RoomAccountProgressOutboxTest` | every commit — report 0.1.496: the same three ids retried 37 times while 120 updates waited |
| Signing out forgets the account figures already acted on, and the identity-bearing tracking sessions | `YouTubeAccountTest` | every commit — covers a revoked refresh token, not only the button |
| An item finished HERE starts again whatever the account says, and marking it unplayed undoes that | `AccountResumePositionsTest` | every commit |
| Whose fault it is that a response carried no tracking — and an odd shape is a null, not a throw | `PlaybackTrackingParserTest` | every commit — the classifier that replaced a design which would have deleted the outbox |
| The downloads menu names "Delete downloaded file", only when there is a copy | `MediaItemRowKeepsActionsTest` | every commit |
| A test run does not post to the diagnostics sink, a hand-sent report still does, and this emulator is recognised as one | `EmulatorsDoNotUploadTest` | every commit — September was 100 emulator reports to 6 real ones; all three cases red-checked on their own mutation |
...
## A probe of the premise is worth more than another test of the fix (2026-09-20)

Report 0.1.496's outbox held 123 updates behind three videos YouTube would give no tracking for.
The obvious fix was to write a row off after a few failed attempts, and it was built, tested four
ways and green.

Then a throwaway live probe asked YouTube about those three ids directly:

```
[probe] Dewi's stuck #1 hgQONOM15HI -> 200 tracking=true refusal=null
[probe] Dewi's stuck #2 ZONm_FuGF5g -> 200 tracking=true refusal=null
[probe] Dewi's stuck #3 86S2CePsIvQ -> 200 tracking=true refusal=null
```

All three had started working. The refusal was **temporary**, so the write-off would have destroyed
real listening — and no amount of testing it would have said so, because every test asserted the
behaviour was correctly implemented, not that the premise was true. The feature was deleted and
replaced with ordering, which loses nothing.

`OneUntrackableVideoDoesNotBlockTheOutboxTest` is what the probe became. Its untrackable id is a
**made-up** one rather than one of the three, precisely because the three can change their minds and
a test built on them would quietly stop testing anything.

### What the rewind work leaves uncovered, deliberately

`RewindIsRememberedTest` drives the real player and the real Room store, but with a **podcast**, so
the account is never involved. `AccountResumeSurvivesTheProcessTest` uses both real Room stores but
a **fake** frozen history. So the full chain — a real video, a real account read, a real seek, and
