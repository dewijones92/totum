...
| JVM | `SmartCutTest` (12) | plus: dropped-chunk gaps never turn speech into not-speech; switched on while audio is flowing it judges what follows (the review's mutant, ignoring the offset, judged 0 frames); a replaced track does no more work; until the detector answers a frame is cut exactly as Standard; a detector on its own thread reaches the same verdicts as inline; a chunk that arrives after its frames were decided is not modelled |
...
- **Smart's extra cut is not capped.** It cuts anything the detector calls non-speech up to a
  ...
  should stay.
- After chunks were skipped as too late, the detector keeps its memory (resetting it was tried and
  measured worse: a cold start calls the first chunks of a word non-speech) but its "not speech"
  verdicts are not trusted for the next six chunks, about 190 ms, so those frames are cut as
  Standard would cut them; "speech" verdicts are always acted on. A test drops alternate blocks
  and asserts no frame the steady detector calls speech comes back as not-speech (9 did before).
