...
| [embedded-player-refusal-falls-back-to-capped-sabr](embedded-player-refusal-falls-back-to-capped-sabr.md) | playback | high | evidenced across two CI runs 47 min apart; the fallback client cannot serve past ~104KB, so the rescue succeeds into a stall. Needs a decision from Dewi |
