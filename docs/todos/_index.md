...
| [subtitles-fail-to-parse-over-sabr](subtitles-fail-to-parse-over-sabr.md) | playback | medium | evidenced (6 failures per affected play, SABR only) and instrumented; cause not yet known, and the subtitle test never touches SABR |
| [embedded-player-refusal-falls-back-to-capped-sabr](embedded-player-refusal-falls-back-to-capped-sabr.md) | playback | high | the hour-long video stalls at 104401 bytes under EVERY client, in three consecutive runs. TWO proposed mechanisms have been disproved; the file holds observations and the control-case discipline, not a cause |
...
- [SABR cannot be opened part-way through](sabr-cannot-seek.md) — a cold jump IS served on the embedded endpoint (2026-09-07, an hour in, 500KB a fetch); what is left is ours: a byte-addressed reader cannot consume a time-addressed answer — the ChunkSource redesign
