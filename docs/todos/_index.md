...
| [subtitles-fail-to-parse-over-sabr](subtitles-fail-to-parse-over-sabr.md) | playback | medium | CAUSE FOUND by the instrumentation: the caption URL asked for no `fmt` while the app declared text/vtt, so YouTube served srv3 XML. Fixed; awaiting a CI run to confirm |
