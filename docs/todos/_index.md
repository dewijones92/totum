...
| [dead-live-test-fixtures](dead-live-test-fixtures.md) | testing | high | two of five fixtures in SabrPlaysAcrossVideoTypesTest no longer exist on YouTube (one removed, one made private), so CI blames the app for someone else's deletion; needs a decision on replacements |
| [subtitles-fail-to-parse-over-sabr](subtitles-fail-to-parse-over-sabr.md) | playback | medium | evidenced (one per play attempt that reaches the parser; SABR route only) and instrumented; cause not yet known, and the subtitle test never touches SABR |
